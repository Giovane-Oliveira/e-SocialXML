                ...
                while (lista.get(x).getE2_FORNECE().replaceAll("\\s+", "").equals(lista.get(x + 1).getE2_FORNECE())) {
                    ...
                    try {
                        ...
                        if ("741".equals(codCategoria.replaceAll("\\s+", ""))) {
                            ...
                            if (lista.get(x).getCBO().replaceAll("\\s+", "").equals("214420") || lista.get(x).getCBO().replaceAll("\\s+", "").equals("223208")) {
                                item2 = "<itensRemun>\n"
    ...
    public void gerarxml1210(List<Dadoss1210> lista, String IndicadorRetificacao, String TipoAmbiente) throws IOException {
        ...
        while (x < lista.size()) {
            ...
            cabecalho = "<eSocial xmlns=\"http://www.esocial.gov.br/schema/evt/evtPgtos/v_S_01_00_00\">\n"
                    ...
                    + "<indRetif>1</indRetif>\n"
                    + //"<indApuracao>1</indApuracao>\n" +
                    "<perApur>" + lista.get(x).getPERIODO().replaceAll("\\s+", "") + "</perApur>\n"
                    + "<tpAmb>1</tpAmb>\n"
                    + "<procEmi>1</procEmi>\n"
                    + "<verProc>1.0</verProc>\n"
                    + "</ideEvento>\n"
                    ...
                    + "</ideEmpregador>\n"
                    + "<ideBenef>\n"
                    + "<cpfBenef>" + lista.get(x).getC9V_CPF().replaceAll("\\s+", "") + "</cpfBenef>\n";

            if (x < lista.size() - 2) {

                while (lista.get(x).getE2_FORNECE().equals(lista.get(x + 1).getE2_FORNECE()) && x < lista.size() - 2) {
                    infopagamento += "<infoPgto>\n"
                            + "<dtPgto>" + lista.get(x).getDT_BAIXA().replaceAll("\\s+", "") + "</dtPgto>\n"
                            + "<tpPgto>1</tpPgto>\n"
                            + //"<indResBr>S</indResBr>\n" +
                            //"<detPgtoFl>\n" +
                            "<perRef>" + lista.get(x).getEMISSAO().replaceAll("\\s+", "").substring(0, 7) + "</perRef>\n"
                            + "<ideDmDev>" + lista.get(x).getIdeDmDev().replace("/", "").replaceAll("\\s+", "") + "</ideDmDev>\n"
...
//"</ideBenef>\n" +
//"</evtPgtos>\n";
                arquivo = "" + cabecalho + "" + infopagamento + "</ideBenef>\n" + "</evtPgtos>\n" + "</eSocial>";
                String data = dtf.format(LocalDateTime.now()).replace("/", "").replace(":", "").replaceAll("\\s+", "").substring(0, 8);
                String hs = dtf.format(LocalDateTime.now()).replace("/", "").replace(":", "").replaceAll("\\s+", "").substring(8, 14);
                String numero2 = gerarnumero();
